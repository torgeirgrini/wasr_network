...
class wasr_IMU_FU2(Network):
    def setup(self, is_training, num_classes):
        ...
        # resize imu to the expected size
        (self.feed('imu_data')
             .resize_img(size_x=64, size_y=48, name='resized_imu_1'))

        (self.feed('imu_data')
             .resize_img(size_x=128, size_y=96, name='resized_imu_0'))
        ...
        (self.feed('res5c_relu',
                   'resized_imu_1')
             .concat(axis=3, name='concatenated_imu_arm_1'))

        # ARM module
        (self.feed('concatenated_imu_arm_1')
             .attention_refinment_module_new(name='arm'))

        (self.feed('res5c_relu')
         .atrous_conv(3, 3, 32, 6, padding='SAME', relu=False, name='fc1_voc12_c0_first'))

        (self.feed('res5c_relu')
         .atrous_conv(3, 3, 32, 12, padding='SAME', relu=False, name='fc1_voc12_c1_first'))

        (self.feed('res5c_relu')
         .atrous_conv(3, 3, 32, 18, padding='SAME', relu=False, name='fc1_voc12_c2_first'))

        (self.feed('fc1_voc12_c0_first',
                   'fc1_voc12_c1_first',
                   'fc1_voc12_c2_first')
         .add(name='fc1_voc12_first'))

        (self.feed('arm',
                   'fc1_voc12_first')
             .feature_fusion_module_new(name='ffm_first', num_features=1024))


        (self.feed('res3b3_relu',
                   'resized_imu_1')
             .concat(axis=3, name='concatenated_imu_arm_2')
             .attention_refinment_module_new(name='arm_2', last_arm=True)
             .conv(1, 1, 1024, 1, 1, biased=False, relu=False, name='arm_2_upfeatures_new'))

        (self.feed('ffm_first',
                   'arm_2_upfeatures_new')
             .add(name='combine_arm1_arm2'))

        (self.feed('res2c',
                   'resized_imu_0')
             .concat(axis=3, name='concatenated_imu_ffm_1'))

        # Extract Likelihood from inbetween layer res2bc
        (self.feed('concatenated_imu_ffm_1',
                   'combine_arm1_arm2')
             .feature_fusion_module_new(name='ffm', num_features=1024))
